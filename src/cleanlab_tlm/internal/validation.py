...
SKIP_VALIDATE_TLM_OPTIONS: bool = (
    os.environ.get("cleanlab_tlm_SKIP_VALIDATE_TLM_OPTIONS", "false").lower()
    == "true"  # noqa: SIM112
)
...
def validate_try_tlm_prompt_response(
    prompt: Sequence[str], response: Sequence[str]
) -> None:
    if isinstance(prompt, str):
        raise ValidationError(
            "Invalid type str, prompt must be a list/iterable of strings."
...
def tlm_prompt_process_and_validate_kwargs(
    ...
    process_and_validate_kwargs_constrain_outputs(
        prompt=prompt, kwargs_dict=kwargs_dict
    )
